...
void CVRP::loadInstance(const std::string& fileName)
{
    ...
    // Copia nós e ajusta IDs para 0-indexed
    this->nodes = scanner->nodes;
    for (int i = 0; i < nodesDimension; ++i)
    {
        this->nodes[i].ID = i;  // Ajusta IDs para 0-indexed
    }

    // Marca depósito corretamente
    if (depotID >= 0 && depotID < nodesDimension)
    {
        this->nodes[depotID].isDepot = true;
        this->nodes[depotID].isAvailable = false;
        this->nodes[depotID].demand = 0;
    }
    ...
    std::cout << "  - Depósito: " << depotID << " (0-indexed)" << std::endl;
}
